...
class EventViewMixin(object):
    ...
    def get_success_url(self):
        return reverse('event_rsvp:rsvp_event_staff')
...
class GuestViewMixin(object):
    ...
    def get_success_url(self):
        return reverse('event_rsvp:rsvp_event_list')
...
class GuestCreateView(GuestViewMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            raise Http404
        if self.request.user in (x.user for x in Guest.objects.filter(event__slug = kwargs.get('event_slug'))):
            #  The user is already registered. Raise an Error page.
            raise Http404
        return super(GuestViewMixin, self).dispatch(request, *args, **kwargs)
    ...
    def form_valid(self, form):
        ...
        post_guest_create.send(
            ...
            event=form.event, is_edit=False)
        return resp
