...
class GuestForm(forms.ModelForm):
    ...
    def __init__(self, event, user, *args, **kwargs):
        print("Event is:", event, "and user is")
        """
        ...
        if user and user.is_authenticated():
            self.user = user
            # Also mark the name and email-field to be uneditable. Not working properly.
            print('J bhaye ni! ')
            self.fields['name'].widget.attrs['read_only'] = True
            self.fields['email'].widget.attrs['read_only'] = True
        else:
            self.user = None
